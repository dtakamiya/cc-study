...
1. **正解選択肢の長さ偏り（複数問、出題の偏り観点）**
   - 何が問題か: 機械的な閾値（正解/他選択肢平均の比が1.6倍超または0.625倍未満）に該当する問題が11問ある。特に顕著なのは以下。
     - `basic-035`（ratio 2.31）、`basic-034`（ratio 1.99）、`basic-038`（ratio 1.98）、`basic-037`（ratio 2.23）、`basic-036`（ratio 1.85）、`basic-039`（ratio 1.76）、`basic-041`（ratio 1.84）、`basic-046`（ratio 1.92）、`basic-030`（ratio 1.65）は正解選択肢が他より長い方向に偏る。
