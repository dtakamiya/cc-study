...
本レポート公開後、対応方針の検討にあたって記載内容を実データと突き合わせて再検証した結果を記録します。**本文は監査時点の記録として変更していません。**

### 再検証で裏付けが取れた項目

以下は `jq` による独立した再集計で数値が一致しました。

- 全448問の領域別・レベル別内訳
- ドメイン間 `id` 重複ゼロ
- 欠番 `feature-065`・`prompt-064`
- 文字数偏りの**領域別件数（7領域すべて一致）**、合計92件（本文「約90問」と整合）
- `prompt-design` advanced の `correctIndex: 1` が 10/20 = 50.0% ちょうど
- `token-efficiency` expert の `correctIndex` 分布 0/4/5/7（index=3 が0件）

### 補正1: 「要修正4件」は実質3件

「要修正」の4件目に挙げた `prompt-064` の欠番は、他3件（事実誤り）と性質が異なります。他3件は学習者に誤った知識を与える問題ですが、欠番は**出題データの内容に影響しません**。

さらに、同じ欠番である `feature-065` を「参考情報（修正不要または優先度低）」に分類しており、**同一の現象を異なる優先度に置いています**。分類基準が一貫していません。

**補正後の扱い**: 欠番2件（`prompt-064`・`feature-065`）はいずれも `data/questions/*.json` を変更せず、`json-schema.md` に「欠番は再利用せず、既存最大連番の次を採番する」旨を明記することで対応します。事実誤りは `feature-043`・`token-043`・`prompt-043`/`044` の**3件**です。

### 補正2: token-efficiency の対象範囲

本文「検討推奨A」の記述には、対象範囲を誤読させる点があります。

- 「特に顕著な例」に挙げた `token-055`（4.56倍）は、同じ節で「後半帯」とした **`token-054`〜`064` の内側**です。帯の外にあるのは `token-039`（3.79倍）のみです
- `token-efficiency` の該当11件のうち、`token-001`（0.60倍）は「コマンド名を答える形式で構造的に不可避」と除外理由が示されていますが、**`token-025`（0.58倍）は言及がありません**

**実測による該当11件の内訳**:

| 方向 | ID | 比率 |
|---|---|---|
| 上限超過（9件） | `token-055` 4.56 / `token-039` 3.79 / `token-057` 2.47 / `token-056` 2.45 / `token-060` 1.90 / `token-054` 1.65 / `token-061` 1.65 / `token-043` 1.63 / `token-042` 1.61 | — |
| 下限割れ（2件） | `token-001` 0.60 / `token-025` 0.58 | — |

### 補正3: 「事実照合は極めて健全」の位置づけ

「448問中、事実誤認として検出されたのは3件のみ」という数字は、分類の仕方に依存します。「検討推奨C」に置いた `feature-030`（agent teams が既定無効）、`token-045`（60%閾値が公式に確認できない）、および「参考情報」の `basic-052`（`/doctor` の診断項目）、`security-024`（`rm` の欠落）は、いずれも**事実の精度に関する指摘**です。分類次第でこの数字は7件になります。

加えて本文が「検査範囲と限界」で認めている通り、`harness-design` expert帯の Engineering Blog 由来の問題群は逐語照合を行っていません。**未照合の範囲を残した状態での「極めて健全」という結論は、検査の網羅性を実際より強く示しています。**

### 補正4: 軽微な誤り

- 「特に顕著な例」で `harness-021`（3.33倍）・`harness-024`（3.31倍）を挙げた直後に「**advanced では15問中13問**」と続くため、この2問が advanced に見えますが、**両方とも intermediate** です
- 本文が参照した `quality-checklist.md` は `correctIndex` 分布を「ドメイン×レベル単位（20区分）」と規定していますが、現在は7領域×4レベルで**28区分**です。本文は正しく「全28区分」と書いており実害はありませんが、参照元ドキュメント側が6領域時代のまま更新されていません

### 本補正を踏まえた対応方針

対応の設計は `docs/superpowers/specs/2026-08-08-question-audit-fixes-design.md` にまとめてあります。上記の補正を反映し、事実誤り3件（+ 欠番は規則明文化）と文字数偏り23問を対象としています。
