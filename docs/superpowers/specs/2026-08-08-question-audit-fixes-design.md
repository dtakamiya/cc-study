...
1. **事実誤り（3件・対象4問）** — 正解や解説が公式仕様と食い違う。学習者に誤った知識を与えるため、対応の必要性が最も高い（レポートは欠番1件を加えて「要修正4件」としているが、本設計では性質の違いから分離した。後述）
...
監査レポートは「要修正4件」としているが、4件目の `prompt-064` 欠番は出題データの内容に影響しない採番の問題であり、他3件（学習者に誤った知識を与える事実誤り）とは性質が異なる。同じ欠番である `feature-065` が「参考情報」に分類されている点も含め、レポートの分類が一貫していない。本設計では欠番を C として分離し、事実誤りは3件（`prompt-043`/`044` は同一の指摘なので対象は4問）として扱う。この補正はレポート末尾の「補正（2026-08-08 追記）」節に記録済み。
