...
ページ間の受け渡しは2経路ある。**何を出題するか**（`mode` / `domain` / `level`）はURLクエリパラメータで渡し、
`quiz-page.js` が `parseQuizMode` で解釈する。**学習データ**（進捗・誤答履歴・直前の結果）は
URLに載せず、ストレージ経由でのみ渡す。以下の3キーを使い分けている。
...
- **復習は進捗に一切書き込まない。** 一度見た問題で合格できるとレベルごとの合格基準（初級は全問正解、他は8問以上正解）のゲートが形骸化する。
