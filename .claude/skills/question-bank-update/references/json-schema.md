...
新規問題を追加する際、この制約を壊す変更は行わない。この10問下限は、削除と追加を**すべて適用した
最終状態**のファイルに対して判定する。削除だけを適用した途中の状態では判定しない。

ただし、これは削除そのものの禁止ではない。`updatePolicy`が`"replace"`の領域では、古くなった
問題の削除が更新手順に含まれる。この場合、ユーザーの承認を得た削除は許される（承認のない削除は
`updatePolicy`の値にかかわらず行わない）。たとえばちょうど10問のレベルで1問を削除し1問を追加する
差し替えは、最終状態が10問となるため許される。削除しても`nextIdSeq`は減らさないため、ID一意性の
制約は保たれる。
