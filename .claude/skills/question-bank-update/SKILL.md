...
description: data/questions/*.json（Claude Codeステップアップ問題集、5領域）を最新かつ正確に保つためのスキル。新規問題の追加、既存問題の監査、Claude Codeの仕様変更による陳腐化した問題の検出の3モードを持つ。「問題集をアップデートして」「新しい問題を追加して」「問題を監査して」「陳腐化した問題がないか確認して」等の発言で使用する。
---

# 問題集アップデートスキル

`data/questions/*.json`（5領域: `basic-operations` / `feature-usage` /
`prompt-design` / `security-permissions` / `token-efficiency`）を、
...
3. 対象領域が5領域全部の場合は、全`data/questions/*.json`を読み込む
...
5. レポート保存専用のサブエージェントに、集約済みレポート本文と実行日を
   ...
   連結したもの。5領域全部の場合は`all`）への保存とgitコミットを委譲する。
